...
(function () {
  ...
  function App() {
    ...
    /* re-render global setelah data live di-reload (route tetap terjaga) */
    var fr = React.useState(0);
    var aw = React.useState(false); var addOpen = aw[0], setAddOpen = aw[1];
    React.useEffect(function () {
      window.__SM_FORCE = function () { fr[1](function (n) { return n + 1; }); };
      window.__SM_TOAST = function (o) { toast(o); };
      window.__SM_ADD_WATCH = function () { setAddOpen(true); };
    }, []);

    /* Auto-refresh berkala: tarik data live tiap 60 dtk (senyap, tanpa toast).
       Dilewati saat tab tersembunyi agar tak boros saat tidak dilihat. */
    React.useEffect(function () {
      var id = setInterval(function () {
        if (!document.hidden && window.SM_reload) window.SM_reload();
      }, 60000);
    ...
    var rightActions = [
      ...
      h(window.IconBtn, { key: "r", icon: "refresh", title: "Perbarui data", onClick: function () {
        toast({ kind: "info", title: "Memperbarui data…", sub: "Menarik harga & berita terbaru" });
        if (window.SM_reload) window.SM_reload().then(function () { toast({ kind: "pos", title: "Data diperbarui", sub: "Harga, berita, dan portofolio tersinkron" }); });
      } }),
