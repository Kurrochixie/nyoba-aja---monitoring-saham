...
(function () {
  ...
  function Portofolio(props) {
    var ts = React.useState(P.history.slice()); var txns = ts[0], setTxns = ts[1];
    var fs = React.useState({ date: "15 Jun 2026", code: "BBCA", type: "BUY", lot: "", price: "", fee: "" });
    ...
    function submit() {
      ...
      var t = { date: form.date, code: form.code, type: form.type, lot: lot, price: price, fee: parseInt(form.fee || "0", 10) || 0 };
      setTxns([t].concat(txns));
    ...
    function del(i) { setTxns(txns.filter(function (_, x) { return x !== i; })); }
    ...
    return h("div", { className: "screen" },
      h("div", { className: "split-main", style: { "--aside": "380px" } },
        ...
        h("div", { className: "grid", style: { gap: 18 } },
          hasData ? KPIs() : null,
          hasData ? Positions(P.positions) : EmptyState(function () { setTxns(P.history.slice()); }),
        ...
        h("div", { className: "card sticky-aside" },
          ...
          h("div", { className: "card-pad", style: { display: "flex", flexDirection: "column", gap: 13 } },
            field("Tanggal", h("div", { className: "select" },
              h("select", { value: form.date, onChange: function (e) { upd("date", e.target.value); } },
                ["15 Jun 2026", "14 Jun 2026", "13 Jun 2026", "12 Jun 2026"].map(function (d) { return h("option", { key: d }, d); })),
              h("span", { className: "chev" }, h(Ic, { name: "calendar", size: 15 })))),
      ...
      h("div", null, txns.map(function (t, i) {
        ...
        return h("div", { key: i, style: { display: "flex", alignItems: "center", gap: 13, padding: "13px 20px", borderBottom: i < txns.length - 1 ? "1px solid var(--line-2)" : "none" } },
          ...
          h("div", { style: { flex: 1, minWidth: 0 } },
            ...
            h("div", { style: { fontSize: 12, color: "var(--ink-3)" } }, t.date)),
          ...
          h("button", { className: "icon-btn", style: { width: 32, height: 32, boxShadow: "none" }, title: "Hapus", onClick: function () { del(i); } }, h(Ic, { name: "trash", size: 15 })));
      })));
  }

  function EmptyState(restore) {
    return h("div", { className: "card" }, h("div", { className: "empty" },
      ...
      h("p", null, "Catat pembelian saham pertamamu lewat form di sebelah kanan, atau muat data contoh untuk melihat tampilan portofolio yang terisi."),
      h("div", { style: { display: "flex", gap: 10, marginTop: 4 } },
        h("button", { className: "btn btn-primary", onClick: restore }, h(Ic, { name: "download", size: 16 }), "Muat Data Contoh"))));
  }
